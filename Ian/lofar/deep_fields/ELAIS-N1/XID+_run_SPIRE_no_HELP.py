...
dir_list = glob.glob('data/fir/SPIRE_no_help/*')
num_done = []
...
if os.path.exists('data/fir/SPIRE_no_help/xidplus_run_{}'.format(taskid))==True:()
else:
    os.mkdir('data/fir/SPIRE_no_help/xidplus_run_{}'.format(taskid))

xidplus.save([prior250,prior350,prior500],posterior,'data/fir/SPIRE_no_help/xidplus_run_{}/lofar_xidplus_fir_{}_rerun'.format(taskid,taskid))
...
with serialize_method_as(SPIRE_cat, None):
            registry.write(SPIRE_cat,'data/fir/SPIRE_no_help/xidplus_run_{}/lofar_xidplus_fir_{}_rerun.fits'.format(taskid,taskid),format='fits')
